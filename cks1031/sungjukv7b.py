import cks1031.sungjukv7bDAO as sjv7dao
...
def readAgainSungJuk(sj):
    ...
    nsj.append(int(input(f'{sj[1]} 학생의 새로운 국어 점수는? ({sj[2]})')))
    nsj.append(int(input(f'{sj[1]} 학생의 새로운 영어 점수는? ({sj[3]})')))
    nsj.append(int(input(f'{sj[1]} 학생의 새로운 수학 점수는? ({sj[4]})')))
    computeSungJuk(nsj)
